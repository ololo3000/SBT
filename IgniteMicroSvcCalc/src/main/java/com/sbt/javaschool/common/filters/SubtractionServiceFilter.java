...
public class SubtractionServiceFilter implements IgnitePredicate<ClusterNode> {
    ...
    public boolean apply(ClusterNode clusterNode) {
        ...
        Boolean multiplicationSvcNode = clusterNode.attribute("subtraction.service.node");
        if (multiplicationSvcNode != null && multiplicationSvcNode) {
            return true;
        }

        IgniteCache<String, CalculatorSvcMetric> cache = ignite.cache(SubtractionService.METRICS_CACHE_NAME);
        CalculatorSvcMetric metric = cache.get(SubtractionService.SERVICE_NAME);
